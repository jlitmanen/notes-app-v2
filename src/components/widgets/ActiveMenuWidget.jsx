...
import { Utensils, BookOpen, X, ExternalLink } from "lucide-react";
  ...
  const renderIngredients = (ingredients) => {
    ...
    if (Array.isArray(ingredients)) {
      items = ingredients.map((ig) =>
        typeof ig === "string" ? ig : ig.item || ig.name || JSON.stringify(ig),
      );
    ...
    <section className="card" style={{ marginBottom: "2rem" }}>
      ...
      {activeMenu ? (
        <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          >
            <h4
              style={{ margin: "0 0 0.5rem 0", color: "var(--primary-color)" }}
            >
              Today ({todayName})
            </h4>
            {todayMeals.length > 0 ? (
            ...
            {viewingRecipe.instructions && (
              <div style={{ marginBottom: "1rem" }}>
                ...
                <div
                  ...
                  dangerouslySetInnerHTML={{
                    __html:
                      typeof viewingRecipe.instructions === "string"
                        ? viewingRecipe.instructions
