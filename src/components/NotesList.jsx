...
import { FileText, List, Utensils, Trash2, Edit2, Plus, Search, Users, X, Check } from 'lucide-react';
...
export function NotesList() {
  ...
  const getIcon = (type) => {
    switch (type) {
      case 'note': return <FileText size={20} color="var(--dark-color)" />;
      case 'list': return <List size={20} color="var(--primary-color)" />;
      case 'menu': return <Utensils size={20} color="var(--accent-color)" />;
      default: return null;
    }
  };
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: '1rem' }}>
          {filteredItems.map(item => (
            <div key={item.id} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', gap: '1rem' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', minWidth: 0 }}>
                ...
                <div style={{ minWidth: 0, overflow: 'hidden' }}>
                  ...
                  <small style={{ color: '#888' }}>
                    {new Date(item.created).toLocaleDateString()} • {item.type.toUpperCase()}
              ...
              <div style={{ display: 'flex', gap: '0.5rem', flexShrink: 0 }}>
                <button 
                  ...
                  style={{ padding: '0.4rem', color: item.groups?.length > 0 ? 'var(--primary-color)' : '#888' }}
          ...
          {filteredItems.length === 0 && <p style={{ textAlign: 'center', fontStyle: 'italic', color: '#666' }}>No items found.</p>}
        </div>
      )}
        ...
        <div className="modal-overlay" style={{
          ...
          <div className="card" style={{ width: '100%', maxWidth: '400px', margin: 0 }}>
            <div className="flex justify-between items-center" style={{ marginBottom: '1.5rem' }}>
              ...
              <button className="secondary" onClick={() => setSharingItem(null)} style={{ padding: '0.2rem' }}>
                <X size={20} />
              </button>
            </div>
            
            <p style={{ fontSize: '0.9rem', marginBottom: '1rem' }}>Select groups to share this item with:</p>
              ...
              {groups.length > 0 ? groups.map(group => (
                <div 
                  ...
                  style={{ 
                    ...
                    borderRadius: '8px',
                    cursor: 'pointer',
                    background: sharingItem.groups?.includes(group.id) ? 'var(--bg-color)' : 'transparent',
                    marginBottom: '0.4rem',
                    border: '1px solid #eee'
                  }}
                >
                  <span>{group.name}</span>
                  {sharingItem.groups?.includes(group.id) && <Check size={18} color="var(--primary-color)" />}
                </div>
              )) : (
                <p style={{ fontStyle: 'italic', color: '#888', textAlign: 'center' }}>No groups found.</p>
              )}
            </div>
